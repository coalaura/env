...
commands["cleanup"] = function()
    ...
    local dirs = {
        temp,
        path.join(utils.home(), "AppData", "Local", "Temp"),
        path.join(utils.home(), ".bun", "install", "cache"),
    ...
    local ff_profiles = path.join(appdata, "Mozilla", "Firefox", "Profiles")

    if os.isdir(ff_profiles) then
        local handle = io.popen(string.format("dir /b /ad \"%s\" 2>nul", ff_profiles))

        if handle then
            for profile in handle:lines() do
                local cache = path.join(ff_profiles, profile, "cache2")

                if os.isdir(cache) then
                    table.insert(dirs, cache)
                end
            end
    ...
    for _, dir in ipairs(dirs) do
        if dir and dir ~= "" then
            local normalized = path.normalise(dir):lower()

            if not seen[normalized] and os.isdir(dir) then
                seen[normalized] = true

                utils.printf("cleaning %s", utils.clean_path(dir))

                -- delete files older than 1 hour
                os.execute(string.format(
                    "powershell -NoProfile -Command \"Get-ChildItem -LiteralPath '%s' -Recurse -File -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -lt (Get-Date).AddHours(-1) } | Remove-Item -Force -ErrorAction SilentlyContinue\"",
                    dir
                ))

                -- remove empty subdirectories older than 1 hour
                os.execute(string.format(
                    "powershell -NoProfile -Command \"Get-ChildItem -LiteralPath '%s' -Recurse -Directory -ErrorAction SilentlyContinue | Sort-Object FullName -Descending | Where-Object { $_.LastWriteTime -lt (Get-Date).AddHours(-1) -and @(Get-ChildItem -LiteralPath $_.FullName -ErrorAction SilentlyContinue).Count -eq 0 } | Remove-Item -Force -ErrorAction SilentlyContinue\"",
                    dir
                ))
    ...
    if cleaned == 0 then
        utils.errorf("no temp directories found")
    else
        utils.successf("cleaned %d directories", cleaned)
    end
end
